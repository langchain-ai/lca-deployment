...
def create_client(deployment_url: str, jwt: str | None = None) -> LangGraphClient:
    ...
    If a JWT is provided, it is sent as `Authorization: Bearer <jwt>` so the
    deployment's `@auth.authenticate` handler can validate it.  Otherwise the
    LangSmith API key is used (and the request fails @auth.authenticate, since
    that handler only accepts bearer tokens).
    """
    if jwt:
