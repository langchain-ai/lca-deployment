...
`python/m5/l4/client.py`, which you will run later in the lesson, maps directly to the diagram — the comments show which steps each line corresponds to:
...
`python/m5/l4/auth.py` replaces the dict lookup with a call to the auth provider. The `@auth.on` handler at the bottom is unchanged from m5.3:
...
Run `python/m5/l4/client.py` to log in as both users and verify they can only see their own threads:
