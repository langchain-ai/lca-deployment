...
`python/m5/l4/setup.py` registers Alice and Bob in your Supabase project. Run it once from a second terminal:
...
<MCQ
    ...
    correctIndex={1}
    explanation="GET /auth/v1/user with the user bearer token in the Authorization header and the service role key in the apiKey header returns the user profile if the token is valid, or an error if not. You can verify this in python/m5/l4/auth.py."
/>
