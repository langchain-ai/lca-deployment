...
<MCQ
    ...
    choices='["The Agent Server itself", "The Auth Provider", "The Client Application", "LangSmith"]'
    correctIndex={1}
...
<MCQ
    ...
    choices='["It is used by the client to log in", "It signs new JWTs for each request", "It is an alternative to the bearer token", "It is used by the agent server to validate tokens via the auth provider API"]'
    correctIndex={3}
...
<MCQ
    ...
    choices='["GET /auth/v1/user", "POST /auth/v1/signup", "POST /auth/v1/token", "GET /auth/v1/validate"]'
    correctIndex={0}
...
<MCQ
    ...
    choices='["It must now call the auth provider to fetch ownership data", "It must validate the JWT before stamping metadata", "Nothing \u2014 @auth.on is unchanged", "It receives a different ctx.user type"]'
    correctIndex={2}
...
<MCQ
    ...
    choices='["The @auth.on handler only", "Both handlers and langgraph.json", "The agent graph itself", "The @auth.authenticate handler only"]'
    correctIndex={3}
