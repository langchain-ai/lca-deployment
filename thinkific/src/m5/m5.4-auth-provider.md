...
<details>
<summary>JWT background</summary>
<br>
...
5. Update the `.env` file in the lab directory. You can copy `.env.example` as a starting point, or copy your existing `python/.env` (or `typescript/.env`) into `python/m5/l4/` (or `typescript/m5/l4/`) and add the three Supabase keys. `langgraph dev` uses the `.env` in the lab directory, as specified by the `"env": ".env"` field in `langgraph.json`.
...
In the `langgraph dev` terminal you can also see each request as it happens. The log mostly shows **outgoing** calls from LangGraph to Supabase — one `GET /auth/v1/user` per authenticated request. Failed incoming requests are logged as warnings. Here is a summary:
...
<MCQ
    ...
    correctIndex={1}
    explanation="GET /auth/v1/user with the user bearer token in the Authorization header and the service role key in the apiKey header returns the user profile if the token is valid, or an error if not. You can verify this in m5/l4/auth.py (or auth.ts)."
/>
