[🔗 For translation, open lesson in new tab and use Chrome translate](https://langchain-ai.github.io/lca-lessons/deployment/m5/m5.2-local-auth.html)
...
<MCQ
    ...
    choices='["Content-Type", "X-LangSmith-Token", "User-Agent", "Authorization"]'
    correctIndex={3}
...
<MCQ
    ...
    choices='["A 401 is returned to the client and the agent never runs", "The request proceeds with an anonymous identity", "LangGraph retries with the LANGSMITH_API_KEY", "The token is stored and retried later"]'
    correctIndex={0}
...
<MCQ
    ...
    choices='["name and email", "role and permissions", "identity", "token and expiry"]'
    correctIndex={2}
...
<MCQ
    ...
    choices='["No, @auth.authenticate isolates users automatically", "Yes, but only threads from the last 24 hours", "No, LangGraph isolates by identity by default", "Yes, until you add @auth.on handlers in the next lesson"]'
    correctIndex={3}
