...
├── auth.ts             ← authentication handler (deployment-wide)
...
## Step 1: Examine the auth file

Examine the auth file at the root of your project. This file holds all of your authentication and authorization logic across these lessons — you will grow it in the following lessons without replacing it.
...
The handler receives the raw `Authorization` header. It splits off the `Bearer` scheme, looks up the token, and either returns a user object or raises a 401.
