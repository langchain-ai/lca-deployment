...
## Step 1: Examine auth.py

Examine `auth.py` at the root of your project. This file holds all of your authentication and authorization logic across these lessons — you will grow it in the following lessons without replacing it.
...
### Alternative: read the user from `runtime`

A node can also receive a `Runtime` object instead of `config`. The same authenticated user is available via `runtime.server_info.user`:
...
Both patterns are supported. The config form has been around longer and works in any node signature; the runtime form is newer and reads more naturally if you are already using `Runtime` for other things (like the Store).
