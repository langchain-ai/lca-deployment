...
<MCQ
    ...
    choices='["The store_namespace \u2014 needed to know which Store entry belongs to this student", "Nothing \u2014 the agent could find any student by searching the Store", "The student_name \u2014 used as a fallback if the Store read fails", "The module_id \u2014 determines which skills to load"]'
    correctIndex={0}
...
<MCQ
    ...
    choices='["No \u2014 the assistant context was set at registration and is not updated", "Only if the student starts a new thread", "Yes \u2014 the Store read happens at prompt time and always returns the latest value", "Depends on whether the Store was written before or after the session was created"]'
    correctIndex={2}
...
<MCQ
    ...
    choices='["ls() and download_files() are replaced with store-specific calls", "The middleware bypasses the backend and reads from the Store directly", "The sources= paths must be updated to use store namespaces", "None \u2014 SkillsMiddleware calls the same BackendProtocol methods regardless of backend"]'
    correctIndex={3}
