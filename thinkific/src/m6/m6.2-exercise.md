...
`deepagents-cli` and `langgraph-cli` are already listed as project dependencies in `python/pyproject.toml` — no separate install needed. They are not on the global PATH, so invoke them via `uv run`:
...
In the next lesson (m6.3) you will point a custom UI — the familiar `deep_tutor` interface — at the agent you just deployed.
