...
The `deepagents-cli` and `langgraph-cli` packages are project dependencies — they're listed in `python/pyproject.toml`. Sync the project venv to install them along with the `deepagents` library they need:
...
You'll invoke the CLI via `uv run` so it uses the project's pinned versions (see m6.1's "Three Commands" section for why).
...
```bash
cd /Users/geoffladwig/Documents/Github/lca-deployment/python/m6/dacli_tutor
cp .env.example .env
```
...
The server starts on `http://localhost:2024` by default. The API is at the root; the bundled frontend is mounted at `/app`. Open `http://localhost:2024/app` in your browser — you should see the LangChain Tutor UI.
